...
if [[ ${FLAG} -eq 1 || ${FLAG} -eq 0 ]] ; then
	...
	## postprocess
	echo -e "${FASTQ_PATH}/*.fastq.gz\n$(zcat ${FASTQ_PATH}/*.fastq.gz | wc -l)" >> 1-alignment-${NAME}.log
	for i in ${FASTQ_PATH}/*.fastq.gz; do echo -e "$i $(zcat $i | wc -l)" >> 1-alignment-${NAME}.log; done
